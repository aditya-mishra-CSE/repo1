...
export default function OtherShapes() {
        ...
        switch (tool) {
            ...
            case "square":
                newShape = {
                    ...
                    startY: pos.y,  // <-- store starting click point
                    x: pos.x,       // initial center x (can be same as startX)
                    y: pos.y,       // initial center y
                    width: 0,
                    height: 0,
                    color: penColor,
                    size: penSize,
    ...
    const handleMouseMove = (e) => {
        ...
        if (tool === "eraser") {
            ...
            for (let i = shapes.length - 1; i >= 0; i--) {
                ...
                if (shape.tool === "rectangle") {
                    ...
                    if (
                        point.x + eraserSize >= left &&
                        point.x - eraserSize <= right &&
                        point.y + eraserSize >= top &&
                        point.y - eraserSize <= bottom
                    ) {
                        erase = true;
                    }
                }
                    ...
                    if (shape.radius) {
                        const dx = point.x - shape.x;
                        const dy = point.y - shape.y;
                        if (Math.sqrt(dx * dx + dy * dy) <= shape.radius + eraserSize) erase = true;
                    }
                }
                ...
                // Cylinder erase check
                    ...
                    if (dx >= -shape.radius && dx <= shape.radius && dy >= -shape.height && dy <= 0) erase = true;

                    const rx = shape.radius, ry = shape.radius / 3;
                    ...
                    erase = dx * dx + dy * dy <= (shape.radius + eraserSize) * (shape.radius + eraserSize) && dy <= 0;
                }
        ...
        switch (lastShape.tool) {
            ...
            case "square": {
                if (!lastShape) break;
                const dx = point.x - lastShape.startX;
                const dy = point.y - lastShape.startY;
            ...
            case "pentagon": {
                ...
                const radius = Math.sqrt(dx * dx + dy * dy);  // distance from start to cursor

                lastShape.radius = radius;
                lastShape.x = lastShape.startX + dx / 2;  // center x between start & cursor
                lastShape.y = lastShape.startY + dy / 2;  // center y between start & cursor
                break;
            }
            case "hexagon": {
                const clampedX = Math.max(0, Math.min(point.x, stageSize.width));
            ...
            case "star":
                if (!lastShape) break;
                const dxS = point.x - lastShape.startX;
                const dyS = point.y - lastShape.startY;
                const radiusS = Math.sqrt(dxS * dxS + dyS * dyS);

                lastShape.radius = radiusS;
                lastShape.x = lastShape.startX + dxS / 2;
                lastShape.y = lastShape.startY + dyS / 2;
                break;

            case "trapezium":
                if (!lastShape) break;
                const dxT = point.x - lastShape.startX;
                const dyT = point.y - lastShape.startY;
                const radiusT = Math.sqrt(dxT * dxT + dyT * dyT);

                lastShape.radius = radiusT;
                lastShape.x = lastShape.startX + dxT / 2;
                lastShape.y = lastShape.startY + dyT / 2;
                break;

            case "rhombus":
                if (!lastShape) break;
                const dxR = point.x - lastShape.startX;
                const dyR = point.y - lastShape.startY;
                const radiusR = Math.sqrt(dxR * dxR + dyR * dyR);

                lastShape.radius = radiusR;
                lastShape.x = lastShape.startX + dxR / 2;
                lastShape.y = lastShape.startY + dyR / 2;
                break;
            ...
            case "cube": {
                ...
                // keep it a square: edge = max axis distance
                const size = Math.max(Math.abs(dx), Math.abs(dy));

                // center between start and current pointer
                lastShape.size = size;
                lastShape.x = lastShape.startX + dx / 2;
                lastShape.y = lastShape.startY + dy / 2;
                break;
            ...
            case "pyramid":
                ...
                // Keep width proportional to height for nice triangle
                const width = Math.abs(dx) * 2;  // base width
            ...
            case "hemisphere": {
                ...
                // radius = distance from start to current pointer (horizontal)
                lastShape.radius = Math.sqrt(dx * dx + dy * dy) / 2;
            ...
            case "cone": {
                ...
                lastShape.radius = Math.abs(dx);        // horizontal base radius
                lastShape.height = Math.abs(dy);        // vertical height
            ...
            <div className="whiteboard-container"
                                    ...
                                    case "square":
                                        return (
                                            <Rect
                                                key={i}
                                                id={shape.id}
                                                x={shape.x - shape.width / 2}   // top-left from center
                                                ...
                                                onTransformEnd={(e) => {
                                                    ...
                                                    // Update width/height from scale
                                        ...
                                        return (
                                            <Group
                                                ...
                                                onTransformEnd={(e) => {
                                                    ...
                                                    setShapes(shapes.map(s => (s.id === shape.id ? updated : s)));
                                            ...
                                            >
                                                ...
                                                {/* Connectors */}
                                                <Line points={[front[0], front[1], back[0], back[1]]} stroke={shape.color} />
                                                <Line points={[front[2], front[3], back[2], back[3]]} stroke={shape.color} />
                                                <Line points={[front[4], front[5], back[4], back[5]]} stroke={shape.color} />
                                                <Line points={[front[6], front[7], back[6], back[7]]} stroke={shape.color} />
                                            </Group>
                                        );
                                    }
                                    case "hemisphere":
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <Line
                                                    points={[
                                                        0, 0,                        // top apex
                                                        -shape.width / 2, shape.height,  // left base
                                                        shape.width / 2, shape.height,   // right base
                                                        0, 0
                                                    ]}
                                                    stroke={shape.color}
                                                    strokeWidth={shape.size}
                                                ...
                                                <Line
                                                    points={[
                                                        0, shape.height / 2,             // inner apex
                                                        -shape.width / 2, shape.height,  // left base (same as outer)
                                                        shape.width / 2, shape.height,   // right base (same as outer)
                                                        0, shape.height / 2
                                                    ]}
                                                    stroke={shape.color}
                                                    strokeWidth={shape.size}
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <Line
                                                    points={[
                                                        0, -shape.height,       // apex
                                                        -shape.radius, 0,       // left base
                                                        shape.radius, 0,        // right base
                                                        0, -shape.height         // close the triangle
                                                    ]}
                ...
                >
                    ...
                    <button
                        ...
                        className={`p-2 rounded-full ${tool === "select" ? "bg-blue-100 text-blue-stageSize.height" : ""
                            }`}
                    >
