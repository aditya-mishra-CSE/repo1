// //Below code works when clicking outside any shape , the transformer get disabled
...
//   return <Image image={image} x={0} y={0} width={1100} height={600} listening={false}/>;
// };
...
//     tr.nodes(node ? [node] : []);
//   } else {
//     tr.nodes([]);  // 👈 clear transformer when no selection or not in select mode
//   }
...
//               height={600}
//               onMouseDown={(e) => {
//     checkDeselect(e);   // ✅ deselect when clicked outside
//     handleMouseDown(e); // ✅ still allow drawing/selection
//   }}
...
//Below code if for shapes and image both
...
//                     // Add uploaded image as a new shape
//                     setShapes((prev) => [
//                         ...prev,
//                         {
//                         id,
...
//                         },
//                     ]);
//                     // 🔥 Auto-select "select" tool and the new image
//                     setSelectedId(id);
//                     setTool("select");
//                     };
//                     reader.readAsDataURL(file);
...
export default function Outside() {
  ...
  const handleMouseMove = (e) => {
    ...
    if (tool === "eraser") {
      // simplistic eraser demo logic
      const erasedShapes = shapes.filter((shape) => {
        if (shape.tool === "rectangle" || shape.tool === "circle") {
          const dx = point.x - shape.x;
          const dy = point.y - shape.y;
          return Math.sqrt(dx * dx + dy * dy) > Math.max(shape.width, shape.height);
        }
        return !shape.points?.some((p, idx) => {
          if (idx % 2 === 0) {
            const x = shape.points[idx];
