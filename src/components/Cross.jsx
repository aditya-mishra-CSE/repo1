...
export default function Cross() {
    ...
    const handleMouseMove = (e) => {
        ...
        switch (lastShape.tool) {
            ...
            case "rectangle": {
                ...
                if (lastShape.x + lastShape.width > 1100) lastShape.width = 1100 - lastShape.x;
                if (lastShape.y + lastShape.height > 600) lastShape.height = 600 - lastShape.y;
    ...
    break;
            ...
            case "ellipse": {
                ...
                lastShape.radiusX = Math.min(Math.abs(clampedX - lastShape.x), lastShape.x, 1100 - lastShape.x);
                lastShape.radiusY = Math.min(Math.abs(clampedY - lastShape.y), lastShape.y, 600 - lastShape.y);
                break;
            }
            ...
            case "polygon": {
                ...
                const maxRadiusX = Math.min(lastShape.x, 1100 - lastShape.x);
                const maxRadiusY = Math.min(lastShape.y, 600 - lastShape.y);
                lastShape.radius = Math.min(Math.sqrt(dx * dx + dy * dy), maxRadiusX, maxRadiusY);
                break;
            }
            ...
            case "cube": {
                ...
                let size = Math.max(Math.abs(clampedX - lastShape.x), Math.abs(clampedY - lastShape.y));

                // Prevent crossing stage boundaries
                if (lastShape.x + size > 1100) size = 1100 - lastShape.x;
                if (lastShape.y + size > 600) size = 600 - lastShape.y;
            ...
            case "sphere": {
                ...
                const dx = clampedX - lastShape.x;
                const dy = clampedY - lastShape.y;
                lastShape.radius = Math.min(Math.sqrt(dx * dx + dy * dy), lastShape.x, 1100 - lastShape.x, lastShape.y, 600 - lastShape.y);
                break;
            }
    ...
    if (lastShape.x + width > 1100) width = 1100 - lastShape.x;
    if (lastShape.y + height > 600) height = 600 - lastShape.y;
                    ...
                    <input
                        ...
                        onChange={(e) => {
                            ...
                            reader.onload = () => {
                                ...
                                const newImageShape = { id, tool: "image", src, x: 100, y: 100, width: 200, height: 200 };
